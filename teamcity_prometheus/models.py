...
class Project(Node):
    ...
    def populate(self, team_city):
        Project.logger.info(
            "Populating project '{}' from TeamCity".format(self.name))
        ...
        for bt in build_types["buildType"]:
            ...
            Project.logger.debug("Adding build type '{}' to project '{}'".format(
                build_type.name, self.name))

            self.append(build_type)
...
class BuildType(Node):
    ...
    def from_dict(data, team_city):
        ...
        BuildType.logger.debug(
            "Getting builds for build type '{}'".format(new_build_type.id))

        builds = team_city.get_builds(build_type_id=new_build_type.id)

        BuildType.logger.debug("Got {} builds for build type '{}'".format(
            len(builds["build"]), new_build_type.name))

        for b in builds["build"]:
            build_by_id = team_city.get_build_by_build_id(b["id"])

            build = Build.from_dict(build_by_id, team_city)

            BuildType.logger.debug("Adding build '{}' to build type '{}'".format(
